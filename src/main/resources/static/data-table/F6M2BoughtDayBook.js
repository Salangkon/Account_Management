...
function tableJournal() {
    ...
    $.ajax({
        type: "GET",
        url: "/api-journal/get-all/UV/" + $('#createBy').val() + "/" + fromDate + "/" + toDate,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (msg) {
            ...
            var tableJournal = $('#tableJournal').DataTable({
                ...
                },
                {
                    ...
                    "mRender": function (data, type, row, index, full) {
                        return '<div style="margin-right: 30px;">' + parseFloat(row.sumCredit).toFixed(2).replace(",", "").replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + '</div>';
                    }
                },
                {
                    'data': 'date',
...
function updateStatus(id, status) {
    $.ajax({
        ...
        success: function (result) {
            window.location.href = "/bought-day-book";
        }
    });
} // end update status
...
function saveCreate() {
    ...
    $.ajax({
        type: "GET",
        url: "/api-journal/generate-dep/UV",
        success: function (msg) {
            if (pass) {
                var insert = {
                    id: $('#id').val(), //ลูกค้า
